...
4. 完全性検証が通れば、ボタン直上に成功トースト（12秒で自動消滅）が出て `L{番号}_{タイトル}.txt` がブラウザ既定の DL 先へダウンロードされる。トーストには human/assistant 件数と鮮度（最終メッセージ日時、「最新まで採れているか」の一目確認用）、警告があれば先頭3件と残数が表示される。
