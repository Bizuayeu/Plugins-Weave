...
3. `window.prompt` で Loop 番号を聞かれる：「フヒト: Loop番号を入力してください（例: L00553）。L00553_タイトル 形式ならタイトルも上書き（省略時はセッション名）。空欄でキャンセル。」デフォルトは空欄——番号は推測しない。**キャンセルまたは空欄で無音終了**（ファイルは出ない）。採番の正典は大環主側にある。
   - `L00553` / `00553` だけ入力 → ファイル名のタイトルはセッション名（会話名）から自動接続（従来動作）。
   - `L00553_自作タイトル` まで入力 → セッション名は使わず、入力したタイトルをそのまま採用（最初の `_` で分割、タイトル内の `_` は保持、ファイル名 sanitize は従来通り適用）。
   - `L00553_`（区切りだけでタイトルなし）→ 誤タイプ安全側でセッション名にフォールバック。
   - `_タイトルだけ`（番号なし）→ キャンセル扱い（番号なしでファイルは作らない）。
