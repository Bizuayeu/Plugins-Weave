...
class TestDigestMerger:
    ...
    @pytest.mark.integration
    def test_merge_individual_digests(
        self, temp_plugin_env: "TempPluginEnvironment"
    ) -> None:
        """マージ処理（重複は上書き）"""
        existing = [
...
class TestProvisionalDigestSaver:
    ...
    def test_save_provisional_append_mode(self, provisional_saver) -> None:
        ...
        second_path = provisional_saver.save_provisional(
            "weekly", second_digests, append=True
        )
...
class TestCLIStdinOption:
    ...
    def test_stdin_option_reads_from_stdin(
        ...
        # DigestConfigをモック
        with patch(
            "interfaces.save_provisional_digest.DigestConfig"
