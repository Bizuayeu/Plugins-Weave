...
分析対象ファイルを Read で全文読み、GrandDigest（確定した知識状態）・ShadowGrandDigest
（未確定の文脈）・Identity file（設定されている場合）と突き合わせて、`## 出力フォーマット`
の JSON を組み立てる。これらのパスは prompt で渡される（`## 必須パラメータ` 参照）。

読むときは、Loop ファイルなら対話の流れと洞察の発展を、Digest ファイルなら overall_digest と
individual_digests の構造を追い、過去のダイジェストとの連続性を確認する。各フィールドの
文字数と用途は `## 出力フォーマット` が唯一の仕様。
...
> 📖 まだらボケの詳細定義は [用語集](../GLOSSARY.md#まだらボケ) を参照
