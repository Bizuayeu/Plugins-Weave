...
`digest_auto` は設定読み込みから推奨アクション提示までを内部で完結させる。Claude が決めるのは、
返ってきた状態にどう応じるかだけ。

| CLI が返す状態 | 推奨アクション |
|---|---|
| 初期セットアップ未完了（config 不在） | `@digest-setup` を実行 |
| ShadowGrandDigest 未作成 | `@digest-setup` を実行 |
| 未処理 Loop 検出 | `/digest` を実行（即終了） |
| プレースホルダー検出 | `/digest` を実行（即終了） |
| 欠番検出（gaps） | 不足ファイルの追加を案内（処理は継続） |
