...
def spawn_waiter(target_time: str, theme: str = "", context_file: str = "", file_list: str = ""):
    ...
    log_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "essay_wait.log")
...
'''
    ...
    script_file = os.path.join(script_dir, "essay_waiter_temp.py")
...
def _windows_list_tasks(known_names: list = None) -> list:
    """List Essay_ tasks and known custom tasks from Windows Task Scheduler

    Args:
        known_names: Additional task names to search for (besides Essay_ prefix)
    """
    result = subprocess.run(
...
def _unix_list_cron(known_names: list = None) -> list:
    """List Essay_ entries and known custom tasks from crontab
    ...
    for line in result.stdout.split("\n"):
        # Essay_プレフィックスまたは既知の名前にマッチ
        if "# Essay_" in line or any(f"# {name}" in line for name in search_names):
...
def schedule_list():
    ...
    # Get from JSON backup first (to build known_names)
    data = _get_schedules()
    known_names = [s["name"] for s in data["schedules"]]

    # Get from OS scheduler with known_names
    if sys.platform == "win32":
        os_tasks = _windows_list_tasks(known_names)
    else:
        os_tasks = _unix_list_cron(known_names)
