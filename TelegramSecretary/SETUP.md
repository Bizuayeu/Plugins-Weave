...
- **リポジトリ（最小1つ）** — Cloud Routine が clone する。役割は2つあるが、**1つにまとめても、2つに分けてもよい**:
  - **基本設定**（`<BASE_REPO>`）— エージェントの `Identities/` と本スキルの `Expertises/TelegramSecretary/`（公開可）
  - **非公開データ**（`<PRIVATE_DIR>`）— 秘書人格 `SecretaryRole.md` と運用 state（個人資産）
  > 公開したい人格論等を公開リポに・機密を非公開リポに**分けるなら2つ**（Weave 運用がこれ）。すべて非公開でよいなら**1つにまとめて可**（`<BASE_REPO>`＝`<PRIVATE_DIR>`）。Cloud Routine の sources は1つでも複数でもよく、**複数なら各リポ名ディレクトリ並列で cwd は親、1つなら cwd はそのリポルート**。
