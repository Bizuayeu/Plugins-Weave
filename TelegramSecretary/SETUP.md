...
**管理表（関係者・依頼・対応知）をリポジトリに永続化する場合**（任意・推奨）— cloud routine は毎回 fresh clone で起動し実行環境は揮発するため、秘書が蓄積した管理表を次回起動へ残すには、**リポジトリの固定ブランチ**に git 永続化します。`init-config` では生成されないので、config.json に以下を追記します（雛型は `templates/config.template.json`）:
...
- `registry_dir`: 永続管理表（individuals/tasks/knowledge）の置き場。**揮発 state（offset/lease/media）の `state_dir` とは別**にし、非公開リポ配下を指す。未設定なら `state_dir` にフォールバック
