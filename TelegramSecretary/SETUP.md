...
- **2つのリポジトリ**（Cloud Routine が並列 clone する）:
  - **人格定義リポ**（`<PERSONA_REPO>`）— エージェントの `Identities/` と本スキルを `Expertises/TelegramSecretary/` に配置（公開可）
  - **非公開リポ**（`<PRIVATE_DIR>`）— 秘書人格 `SecretaryRole.md` と運用 state（個人資産ゆえ非公開）
...
marketplace からインストール、または人格リポの `Expertises/TelegramSecretary/` に配置します。**Cloud Routine は人格リポを fresh clone する**ので、`Expertises/TelegramSecretary/`（コード一式）が人格リポにコミットされていることが必要です。

### ④ 秘書人格を用意（SecretaryRole.md）

雛型 [`templates/Identities/SecretaryRole.template.md`](./templates/Identities/SecretaryRole.template.md) をコピーし、**非公開リポの `Identities/SecretaryRole.md`** として、秘書の固有名・対応原則・触れない話題などを定義します（人格は個人資産ゆえ非公開リポに置き、配布物には焼き込みません）。
...
- `--agent-name`: 人格リポの `Identities/<agent_name>Identity.md` を解決する名前
- `--private-dir`: cwd（2リポ親）起点での非公開リポのパス（例 `<PRIVATE_REPO>/TelegramSecretary`）

> config.json は**人格リポの `Expertises/TelegramSecretary/config.json` に置き、Cloud Routine が fresh clone で読めるようコミット**します（秘匿を含まない運用設定ゆえコミット可）。配布リポでは `.gitignore` 対象なので、運用リポ側で明示追跡してください。
...
- **sources は人格リポ＋非公開リポの2つ**
- prompt body 内の `<PERSONA_REPO>` / `<PRIVATE_DIR>` は schedule が自動で実リポ名に置換します（手置換不要）
...
| Step 0 でパス解決失敗 | 2リポ配置の不整合 | sources に人格リポ＋非公開リポの両方があるか、config の `private_dir` が cwd 親起点か確認 |
