...
4. **body 骨子（TS 固有の登録内容。汎用スキーマは `schedule` skill 正典）**：
   ...
   - `job_config.ccr.session_context.outcomes`: **`registry_sync` 有効時のみ**、管理表の push 先 `registry_branch`（既定 `claude/ts-registry`）を git 書き戻し先として宣言する（cloud routine が固定ブランチへ push を許可されるための配線。NewsCaster / BlueberrySprite と同型。`registry_sync` 無効なら outcomes 不要）。スキーマ形式は `schedule` skill / `MEMORY: reference_remote_trigger_update` を正典参照
