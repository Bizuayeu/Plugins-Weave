...
- **記録は送信と同じ手順の内側で終える（WAL の守備範囲外）**: 送信は相手の反応という外部検定を持つが、**台帳への記録は誰も検定しない**——落ちても誰も気づかない。ここは WAL が守らない（判定は key の存在だけで payload を見ないため、**既存レコードへの追記**は redo されず、むしろ「registry に在る」を根拠に done 化される。DESIGN §3.7）。「送った」と「記録した」を別の機会に分けず、送信の直後に記録まで終える
