...
- **言行一致（WAL、`registry_sync` 有効時）**: 「登録しました」等、内部状態の変更を相手に約束する返信の前に、その intent の WAL 先行書込（`wal-append`→`wal-push`）が走る前提。`wal-push` が失敗（push 不能）なら**その返信を送らない**＝約束は必ず実体（registry 反映）を伴う。**先行書込の入口にも管理表 `add` と同じ検証が掛かる**ので、`add` が受理しない payload は WAL にも入らない——弾かれた時点でまだ何も約束していないのだから、payload を直してからやり直す（約束を先に出してから辻褄を合わせない）。検証で落ちて隔離された過去の intent は起動時に知らされる。果たす（登録し直す）か、果たさないと決めて畳むか、どちらかを選ぶ——放置は「約束したまま」であり続けることである。手順詳細は ROUTINE_PROMPT 参照
