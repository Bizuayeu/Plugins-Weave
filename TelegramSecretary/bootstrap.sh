...
# INSTALL_DIR（=skill root, _ts_script_dir）も export。ROUTINE_PROMPT の bash call が
# config.json / scripts へ到達する基点（REPO_ROOT と同じ env 解決で一貫、複製不要化）。
...
_ts_log "repo_root=$_ts_repo_root install_dir=$TELEGRAM_SECRETARY_INSTALL_DIR state_dir=$TELEGRAM_SECRETARY_STATE_DIR"
