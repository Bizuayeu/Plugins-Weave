...
> 設計の背骨は §2 / §3.8 の踏襲——役割判定・3表 CRUD・WAL は決定論（`REGISTRY_SPEC` に3エントリ足すだけで全機構が付く）、役割の演じ方・占術の解釈・伴走の温度は重要度の世界（SecretaryRole）、起動時の7表ロード＋role-status は従属度の世界（ROUTINE_PROMPT）。
