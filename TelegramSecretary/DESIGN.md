...
> 消化（handoff → knowledge への結晶化）と卒業（archive）のサイクルは**本設計のスコープ外＝次段**。現在は分離のみが天井であり、`registry_cli._read_handoff_blocks` に `cc-defer` として昇格トリガー（orientation 実測 100KB 超）を刻んである。
