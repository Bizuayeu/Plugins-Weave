...
| `role-status` | PROFILE/GOALS から現在の役割（secretary/butler/coach/anego＝守護霊）を決定論導出し JSON 1行で emit（P=principal の PROFILE≥1、A=active な GOALS≥1。役割の自称をしない＝判定はコード、演技は SecretaryRole。DESIGN §3.11）。起動時オリエンテーションが1回叩く | 0=OK |
