...
- [3. データアーキテクチャ（管理表 + Identities）](#3-データアーキテクチャ管理表--identities)
  ...
  - [3.13 なぜ presence-only か・どこに掛けるか（裸数値 lint）★lint-numbers 設計根拠 SSoT](#313-なぜ-presence-only-かどこに掛けるか裸数値-lintlint-numbers-設計根拠-ssot)
...
| **PROFILE** | `--profile-cap`（件数絞りは持たない。索引化で有界、v1.19.0） | 索引行の `content` 頭の幅（人物理解の一覧ゆえ母数は減らさない） |
| **ABILITIES** | `--abilities-cap`（件数絞りは持たない。索引化で有界、v1.19.0） | 索引行の `guidance` 頭の幅（能力カタログゆえ母数は減らさない） |
...
- **なぜ category（コード）と subjects（データ）を分けるか（設計核）**: `category`（認識の型：observation / research / …）は**閉じた語彙**——増えないことに意味があり、増殖と綴り揺れこそが v1.8.0 で許可集合検証を入れた理由。対して主題は**開いた語彙**——扱う領域が増えれば増える。これをコードの定数に置くと、主題を 1 つ増やすたびに deploy が要る。**閉じた語彙はコード（frozenset）、開いた語彙はデータ（管理表）**、が分岐の判定基準。両者は直交する軸なので、同じ知見が「observation（型）× 馬（主題）」の両方から引ける
...
- **なぜ CRUD/WAL/スキーマを持たせないか（本質）**: 成果物は「どう構造化するか」自体が秘書の判断（重要度の世界）。固定スキーマや CRUD subcommand を与えると、管理表の決定論性に成果物の非定型性が混入する。`artifacts/` は **場所（`registry_dir/artifacts/`）と git 永続対象であること**だけを標準化し、ファイル構成・命名・索引（INDEX 等）は秘書に委ねる——実例として `mba50` は「章別 md ＋ INDEX」→「単一 JSON マスター」と形を変えており、**スキーマレスこそが要件**であることを示す。§3.5「情報の持ち方は情報の主体が決める」・§2 三世界分類の踏襲
- **なぜ永続か**: 成果物は蓄積が本質（過去の成果物は資産）。§3.6 の git 永続（orphan ブランチ `claude/ts-registry`）に、全管理表・`wal/` と並べて `artifacts/` を載せる。揮発してよい `state_dir` とは永続要件が正反対
- **なぜ backup はツリー同期か（ファイル固定でない）**: 管理表9点（8表＋WAL）が「単一ファイルの状態 SSoT」ゆえ固定列挙でコピーするのに対し、`artifacts/` はファイルが増減する成果物層ゆえ **ディレクトリ単位のツリー同期**（`/ts-registry-backup` が `ls-tree -r` で全ファイル列挙・`rm -rf` で stale 削除も反映）。配布先に `artifacts/` が無ければ空ループ＝no-op（母集団スコープ安全）
...
- **タスク id トークン（v1.17.0、命名を読むだけの二つ目の規約）**: `orientation` の `## artifacts` 節は `artifacts/` 配下（`handoff/` を除く）のパスを列挙し、パス中のタスク id トークン（`t0007/` ディレクトリ・`_t0013_` ファイル名成分）で群にして、active タスクの群だけファイル名を並べる。これは秘書が自然発生させた命名（`t0007/`、`drafts/<日付>_t0013_*`）を**読むだけ**で、ディレクトリ構成もスキーマも与えない——中身は開かず（退行テストで `Path.open` 不在を固定）、トークンの無い legacy は `untagged` として件数だけ載る。なぜ載せるかは §3.12（監査証跡と採択済みの読み口の非対称）。標準化の線は handoff と同じ「置き場と命名」の内側にある
...
### 3.11 なぜ P×A 直交2軸で役割が進化するか（PROFILE/GOALS/STEPS・データ駆動判定・占術スキル接続）

秘書の役割拡張を「パーソナライズ（P）」と「伴走（A）」の**直交する2機能軸**として持ち、その組み合わせで役割名を導出する——秘書（P✗A✗）／執事（P✓A✗）／コーチ（P✗A✓）／守護霊（P✓A✓）。機能の実体は3表の追加（PROFILE=P軸データ、GOALS/STEPS=A軸データ）であり、役割は**フラグではなくデータの状態**から立ち上がる。
...
- **なぜ役割判定をデータ駆動（決定論）にするか**: P = 「PROFILE に subject=principal が1件以上」、A = 「GOALS に status=active が1件以上」を `derive_role` 純関数（Domain）が計算し、`role-status` subcommand が JSON で返す。LLM に「あなたは守護霊ですか」と聞かない——**役割の自称はハルシネーションの温床**（できない能力の宣言と同型）であり、判定をコードに置けば4象限の単体テストが書ける（Testability 最優先、§2 の分離の踏襲）。「どの役割か」はコード、「どう演じるか」は SecretaryRole ガイダンス（重要度の世界）
- **なぜ卒業が起きる設計か**: 全目標が achieved/abandoned になると A 軸が降り、守護霊→執事へ自然に戻る。これは仕様——伴走は預かりものであり、変容を見届けたら手を離す。目標を増やせば再びコーチ/守護霊に戻る（可逆）
...
- **占術スキル（P軸聴取経路①）の接続**: 占術スキルの接続は ABILITIES 経由（`skill_path` に Expertises 実体を登録、§3.8 の自己追記ガードは実在 Read 確認で充足）。配布版 ShioriSecretary では同スキルをリポ同梱して配布物単体で実在検証可能にしている
...
- **出力の有界性（なぜサイズが読めるか）**: ダイジェストは全文でなく**射影**であり、出力量は概ね
  ...
  ```
  小表全文（goals）
      ※ v1.10.0 以降 goals も cap で頭打ちにできる
    + individuals 件数 × 索引行（context_notes 頭は individuals_cap で丸め、v1.18.0）
    + profile / abilities 件数 × 索引行（content / guidance 頭は各 cap で丸め、v1.19.0）
    ...
    + outbound 2 行（WAL の done 最新 created_at ＋本文 topic_width(120B)、pending 件数。v1.16.0）
    + active タスク数 × min(成果物数, artifacts_latest) × パス 1 行 ＋ other 1 行（v1.17.0）
  ```

  で抑えられる。**支配項だった「notes 総長」と「knowledge content 総量」が式から消えている**ことが本質。outbound 行は WAL の**射影**であって WAL 全文ではない——「最後の送信確定はいつか」は決定論なので、毎枠の WAL 手読み（存在しないフィールド名で読む・pending を送信済と読む、の二事故が実際に出た）をコード側へ移した。判定（今日の分を出したか）は秘書に残す——レコードが太っても出力は太らない。**幅の単位は UTF-8 バイト**（v1.7.0 で是正）——退避の閾値がバイトである以上、幅を字数で数えると日本語 1 字≈2.47 バイト（orientation_datafit_20260809 実測）の分だけ実効幅が膨らみ、実データで既定のまま 99,037 バイトを出して沈黙失敗が再発した。**数値は不変のまま単位だけを是正**してある——v1.5.0 の校正意図（notes 末尾に 3,000–4,000 字の申し送りが堆積する／topic は識別可能な幅で足りる）は「1 字＝1 バイト」の世界で立てた見積りゆえ、バイトとして読み直せばそのまま生きる（120B≈48.6 字・4000B≈1,619 字、日本語主体データでの出力見積は 55KB 圏）。丸めは文字境界で止め、マーカー（"…"＝3 バイト）は幅の内側に置く。handoff の 2 値は実測からの外挿ゆえ仮置きで、いずれも CLI オプションで上書きできる（`handoff_latest` は件数＝単位是正の外）
...
- **監査証跡と採択済みの読み口を揃える（`## artifacts`、v1.17.0）**: tasks の `notes` は追記のみで撤回を書かない**監査証跡**である——置き換えられた基準が断定形のまま印もなく残る。一方、採択された結論は成果物（HTML 等）にあるが、それは索引の外だった。分けて置いてあっても**起動時の射影に載る側**に読み手は落ちる（2026-09-12、T0020 の気学の答えを notes の第 1 基準で返して訂正）。処方は「成果物の中身を載せる」ではなく**索引をもう一つ、notes の直後に置く**——active タスクごとの成果物ファイル名だけを並べ、notes から引いた値を外へ出す前に採択済みを `Read` する読み順を配置で作る。中身を載せないのは §3.10（スキーマレス・解釈しない）と有界性の両方から。規律を knowledge に書くだけでは効かない（上流配置の原則）ので body に置いた
- **knowledge の検索口（`knowledge search`、v1.17.0）**: 新しい知見を焼く前の既出照合は、索引（`id | subjects | topic`）では足りない——索引に無い語で書かれた既出は content の中にしか無い。口が無いと秘書は自前の部分文字列走査を全件に書く（27 語 × 457 件、2026-09-12）ことになり、件数に比例して結晶化が遅くなる。検索は read-only の純関数（NFKC → casefold の部分文字列一致、AND／OR）で、出力は索引行に限る（本文は `get --key`）——list と同じ沈黙失敗を新しい口で作らないため、見出しに母数を開示し、総バイトを stderr に申告し、閾値超は絞り方を添えて警告する。0 件は観測結果であってエラーではない
...
- **卒業すると参照の解決先が二箇所になる（辿る側の規律）**: notes や knowledge に書かれた handoff への参照は**ファイル名だけ**（`20260814T111500Z_session-xxxxxxxx.md`）でディレクトリ成分を持たない。名前は不変だが場所は動くので、卒業後の解決先は「`handoff/` 直下 → `handoff/archive/`」の二箇所になる。**直下に無い＝失われた、ではない**——archive まで見てから判定する。参照側にパスを焼き込んで解決する手もあるが、それは卒業のたびに全参照を書き換えることになる（名前で引ける利点を捨てる）ので採らない。**orientation に載らないこと（読み筋の外）とファイルが無いことは別**、を混同しないための規律
...
- **profile / abilities も索引へ移した（v1.19.0）**: 同じ誤読が残っていた。profile は「principal 1 件」の前提で cap 側に置いたが、P 軸は人物理解が深まるほど育つ表で、2026-09-25 には 4 件・4,772 バイト（1 件≈1,180）になっていた。cap が縛るのは `content` だけで、`traits[]` / `sources[]` / timestamps と JSON の骨格は素通り——1 レコードの描画長が「そのレコードに誰が何を書いたか」で決まり、上限が無かった。処方は individuals と同じ基準で列を選ぶ: profile は `id | subject | method | traits | content`（**traits は応答調整に引く特性タグ＝個票を引く前でも要る側**なので全量、`sources` は出所のポインタなので `get --key` の側）、abilities は `id | name | trigger | skill_path | guidance`（**trigger は発動シグナル**なので全量、`related` はポインタ）。丸めるのは `content` / `guidance` だけで、**`--profile-cap` / `--abilities-cap` は引数名を保ち意味だけを頭の幅へ移した**（v1.18.0 と同じ互換の理由。未指定は 120）。登録済みの引数のまま 1 件あたり profile 約 1,180 → 約 700 バイト（実測は CHANGELOG v1.19.0）。**全量で載せる列（traits・trigger）はなお有界ではない**——上限を持つのは丸めた 1 列だけで、行の残りは固定列の長さと特性タグの数で決まる。goals は 0 件で列を実測から選べないため cap 側に残した（`_CAP_FIELDS` の `cc-defer`）
...
- **表の性質が処方を決める（8 表の処方一覧）★SSoT**: 蓋の掛け方は表ごとの気分ではなく、**1 レコードが長いのか／レコード数が増えるのか**で決まる。前者には cap（支配的長文フィールド 1 つだけを丸める）、後者には索引または件数絞り（行あたりを最小化し、母数は見出しで開示する）。新しい表が生えたら、この二択のどちらかに必ず入れる——どちらでもない表は「蓋の無い表」であり、育ったときに沈黙失敗を再発させる側になる。
  ...
  | abilities | 索引（`id \| name \| trigger \| skill_path \| guidance`、件数絞りは付けない）＋`--abilities-cap`（`guidance` 頭の幅） | 能力は増える（v1.19.0 で cap から移した）。`trigger` / `skill_path` は発動判断に要るので丸めない |
  | profile | 索引（`id \| subject \| method \| traits \| content`、件数絞りは付けない）＋`--profile-cap`（`content` 頭の幅） | 人物理解は深まるほど育つ＝件数が増える（v1.19.0 で cap から移した。「principal 1 件」の前提は 4 件で崩れた） |
  | goals | cap（`notes`、`--goals-cap`） | 件数は少なく本文が長い（0 件ゆえ索引の列を実測から選べない、v1.19.0 で据置） |
  | steps | 索引（`id \| goal_id \| seq \| status \| title`）＋`--steps-latest` | 目標からの逆算単位ゆえ設計上 `done` が高速に溜まる＝件数が増える |

  実装上、この表は 3 箇所に写っている——`_CAP_FIELDS`（cap 側 1 表への経路の写像）、`_INDEXED_SMALL_TABLES`（索引で載せる小表 3 表の列・鍵・行の射影）、`_table_section` の分岐（残る索引側 4 表）。**「既定は全文」の分岐に残るのは cap 側 1 表**（goals）で、`_CAP_FIELDS` のキー集合とちょうど一致する。subjects に cap でなく索引を採ったのは、cap が 1 レコードの長さにしか効かず語彙の件数成長に無力だから——全文 JSON は `created_at` / `updated_at` という**この用途で価値ゼロの数十バイト**を毎レコード運ぶ
...
### 3.13 なぜ presence-only か・どこに掛けるか（裸数値 lint）★lint-numbers 設計根拠 SSoT

数値に出所（実測なのか見込みなのか継承なのか）が添えられていないと、読み手はどの数を信じてよいか決められない。`lint-numbers` はこの「裸の数値」を機械で拾うが、**拾うのは presence だけ**である。対象は**納品物と handoff の両方の原稿 md**（v1.15.0 は納品物のみ、v1.15.1 で handoff へ拡大——経緯は本節末尾）。いずれも消極的な設計判断ではなく、実測から出た線である。
...
- **導入時なぜ納品物からか（誤読コストの非対称）**: handoff 側は分析ノート運用（案2）が現に効いていた——分母が 30〜50 帯から 84/96/104 へ増えても裸は 4〜8 のまま抑えられた。一方**納品物側は読み手が大環主とその先の第三者**であり、誤読の costs が自分に返ってこない。加えて納品物の裸率は一度も測られていなかった。**網はまず誤読コストの非対称な側に張った**
- **handoff への拡大（v1.15.1、2026-09-02 裁可）**: 当初は「裸率 20% 帯再突入」を昇格トリガーとする cc-defer だったが、トリガーを待たず前倒しで拡大した。判断の根拠は役割分担の明確化——**降下を起こす作動成分は裸箇所の名指し分析であり、機械はその当たり付けを安くする**。handoff の書き手は毎枠交代する（注意は構造的に逸れる）ため、注意が逸れた枠で気づかせる装置は handoff 側でも同じに働く。**分析ノート（名指し）が本体・lint は当たり付けという分担、納品物側と handoff 側の裸率を別々に報告する規律、ゼロを目標に置かない前提は、拡大後も不変**——lint が緑でも名指し分析の代わりにはならない（緑の危険は下の偽陰性の項）
...
- **なぜ原稿 md 側に掛けるか**: 納品物は md 原稿と html の対で作られる運用実態がある。生 HTML に行走査を掛けると属性値・px・hex 色でほぼ全行が候補化し、**ノイズに慣れて緑を読み飛ばす**状態を自前で作る（T0015 が案1の難点として挙げた再現）。タグ剥がしは判定の入口なので実装しない——**HTML しか無い納品物で候補ノイズが出るのは仕様**とし、原稿 md に掛けるのを正規手順とする（handoff はもとより md なのでそのまま掛かる。手順は ROUTINE_PROMPT）
