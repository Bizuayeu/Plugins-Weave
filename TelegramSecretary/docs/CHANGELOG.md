...
1 日 6 回）。存在しないフィールド名で読もうとした枠、「outbound が 1 行ある＝今日の分は送信済」と
誤読しかけた枠が実際に出た。判定は秘書の仕事のままでよいが、「最後に送信が確定した outbound は
いつか」は決定論なので、ダイジェストへ 2 行足して手読みを消す。
...
- **`orientation` の counts 直後に `## outbound` 節** — WAL の outbound kind を射影し、
  ...
  最新が取れる。無ければ `none`）と `pending: N`（intent は書いたが送信未確定＝時刻を載せない。
  載せると送信済と読まれる）を出す。見出しに done/pending の意味と「done は `wal-redo` の
  checkpoint が retention で掃除する」ことを開示する——`none` は「retention 内に送信確定なし」で
  あって「一度も送っていない」ではない。WAL が読めない枠は stderr に `outbound wal unreadable`
  を出して `none` で続行する（handoff の読み筋と同じ fail-open）。UseCase は `summarize_outbound`
  （純関数）、配線は `registry_cli._read_outbound_entries`（read-only、git に触れない）。
  ROUTINE_PROMPT Step 5 の項目 10 に「この行から判定し、WAL を手読みしない」を足した
...
- `ROUTINE_PROMPT.md` — 送信前チェックに「納品物は**原稿 md** に `lint-numbers` を掛け、
  裸の行に計器を書いてから送る」を追加（send-reply / proactive-send の両経路）。
  **納品物側と handoff 側の裸率は別々に報告し、合算しない**（母集団が違う）
- `STRUCTURE.md` — `domain/` に `number_lint.py`。併せて既存の記載漏れ
...
## [1.13.0] - 2026-08-25 — strict を本番だけに掛ける：1294 の 7% が本体だった

前版で型検査を配線し 18 件を 0 にしたが、`strict = true` は入れていなかった。入れると
**1294 件**出る——という数字が「まだ早い」という見え方を作っていた。

### 機序

**1294 件のうち本番コードは 92 件（7%）だけだった。** 残る 1202 件はテスト層で、内訳は
no-untyped-def 938 / no-untyped-call 213——中身は test 関数への `-> None` 付与という
機械的増補である。バグ検出への寄与はほぼ無く、双子（ShioriSecretary）との diff 監査面積
だけが増える。テストの担保は pytest が持つ。

**strict の値打ちは本番側にある。** 型検査を入れる動機になった不具合（`os.path.join` →
`Path(a) / b` の機械変換で str が Path に変わり `sys.path` へ流れる）は本番コードに居り、
テストが緑のまま通り抜けた種類のものだった。テスト層を除外しても、その網は閉じたままになる。
...
`_import_records` が既に持っている）。dict へ狭める修正を一度入れて import 系 7 件が赤くなり、
差し戻した。

### 検証

mypy 92→0 ／ ruff All checks passed ／ pytest 860 passed（前後一致）
...
- **ポーリング窓の不変条件が最悪滞留を過小に見積もっていた（SIGTERM の実因）**——窓の既定値 540s は「1 サイクルの最悪滞留 = long-poll の `--timeout`（30s）」という前提の上に立っていたが、実際に窓を越えさせるのは HTTP 層の再試行予算のほうだった。`watch` は最終サイクルの long-poll を残り窓へ丸めるが（`main.py` の `poll_timeout`）、**丸まるのは long-poll の引数だけ**で、その内側の `api_gateway`（`retry_count=2` / `request_timeout=40.0`、**5xx は sleep せず即再試行**＝待ちではなく試行回数が時間を食う）は窓を知らない。ゆえに最悪滞留は 30s ではなく `(2+1) × 40 = 120s` で、`540 + 120 = 660 > 600` と破れる。平常時は 502 が出ないので `540 + 30 = 570 < 600` が成立して見えていた——**障害時にだけ露出する条件**だった。2026-08-15、502 が続いた枠が exit 143（SIGTERM）で落ちて発覚（`server error after retries: 502` が 8 行、`--exit-on-message` の 1 コール = 1 サイクル規律は守られていた＝既知の壊れ方ではない）。**窓の既定値を 540 → 450 に下げた**（`450 + 3×40 + 30(後処理余裕) = 600`）
- **同じ腐りをコメントで止めない**——窓（`bootstrap.sh`）と再試行予算（`api_gateway`）は別ファイルに住んで互いを知らないので、不変条件を注釈に書いても片方だけ動かせば静かに破れる（現に破れた）。`test_poll_window_invariant.py` が**両方の実値を突合**し、片側を動かしたら赤になるようにした（上限を運用手続きではなくテストの失敗として実装する、の踏襲）

### Changed

- **DESIGN §3.7 に WAL の守備範囲を明記**——`reconcile` / `settle` の判定は `(kind, key)` の存在だけで payload を見ないため、**既存レコードの内容更新（例: 既にある tasks の `notes` 追記）は WAL では守れない**。intent を積んでも key は既に在るので redo されず、それどころか `settle` が done 化して**反映されていないのに反映済みに見える**（偽陽性）。WAL が塞ぐのは「約束したレコードが無い」穴だけで、「約束した内容が入っていない」穴は順序（送信と記録を同じ手順の内側で連続させる）でしか塞げない——この境界が書かれていなかったため、WAL があるから守られていると読める状態だった
- **DESIGN §3.12・SKILL に「卒業後の参照は二箇所で解決する」を明記**——申し送りへの参照はファイル名だけでディレクトリ成分を持たず、`handoff-archive` は名前を変えずに場所だけを動かす。ゆえに卒業後の解決先は「直下 → `archive/`」の二箇所であり、**直下に無い＝失われた、ではない**。orientation に載らないこと（読み筋の外）とファイルが無いことは別、という判別を辿る側の規律として置いた
- **ROUTINE_PROMPT の不変条件の記述を実際の式へ改めた**（`max_duration + (retry_count+1) × request_timeout + 後処理余裕 <= bash_timeout/1000`）

### 移行（稼働中の routine への波及）

1. **窓の是正は body 再登録を要さない**——登録済み body は `$TELEGRAM_SECRETARY_POLL_SET_SEC` を参照するだけで、値は `bootstrap.sh` が持つ。routine は毎枠 fresh clone するため、**本体リポ（`Homunculus-Weave/Expertises/TelegramSecretary`）へ反映された次の枠から 450 で起動する**（正典 `plugins-weave` だけ直しても稼働には届かない）。Routine の Environment で `TELEGRAM_SECRETARY_POLL_SET_SEC` を明示注入している場合はそちらが勝つので、注入していないことを確認する
2. SKILL.md も毎枠読まれるため、卒業後の参照規律は同じ経路で届く。**ROUTINE_PROMPT の記述だけは body 再登録が要る**（式の説明であって値ではないので、次のまとめた再登録で足りる）
3. 副作用: 窓が縮む分アイドル枠のポーリング回数が増える（4h 枠で 26 → 32、`MAX_TURNS` は算出式に窓が入るので 86 → 92 へ自動追従）
...
**再発した**——既定の orientation が 99,037 バイトを出し、persisted-output へ退避される。
根っこは「1 文字＝1 バイトの世界で校正された既定値」×「日本語 1 文字≈2.47 バイト」の
**単位不一致**だった。退避の閾値がバイトである以上、幅もバイトで数えなければ有界式は
上限と噛み合わない。あわせて、幅を絞ろうとして `0` を渡すと既定値へ逆転する罠を塞ぐ。

### Fixed

- **CLI オプション `0` が既定値へ逆転する falsy 罠** — `--notes-tail 0` / `--topic-width 0` / `--handoff-latest 0` / `--handoff-cap 0` は「全捨て」の意図で渡されるのに、`or` による解決が `0` を未指定と読んで既定値（4000 / 120 / 3 / 8000）へ差し戻していた——**絞るためのオプションが、最も強く絞った時だけ効かない**。`_option` ヘルパー（`value is None` 判定）を Interface 層に 1 個置いて 4 箇所を置換し、`0` が最小方向の端点として単調に効くようにした。UseCase 側が既に持っていた「非正＝全捨て」ゲートへ値が届く

### Changed

- **丸めと既定値の単位是正（字数 → UTF-8 バイト）** — `_truncate` / `tail_notes`（および handoff cap 経路）が UTF-8 バイトで数え、**文字境界で丸める**（マルチバイト文字を割らない）。マーカー `…`（3 バイト）は幅の内側に置く。既定値は **数値そのまま単位をバイトへ**（topic_width 120B / notes_tail 4000B / handoff_cap 8000B）——v1.5.0 の校正意図は「1 字＝1 バイト」の世界で立てた見積りゆえ、バイトとして読み直せばそのまま生きる（120B≈48.6 字・4000B≈1,619 字）。新数値を発明していない。`handoff_latest=3` は件数ゆえ単位問題の外
- **出力見出しの単位表記** — `tasks.notes (active only, last N bytes)` ほか、幅を示す見出しを `chars` から `bytes` へ改めた
- **DESIGN §3.12（起動時オリエンテーション SSoT）** — 有界式の幅にバイト単位を明記し、「120 字で識別可能」の根拠文を実測（1 字≈2.47B）に基づく新根拠へ差し替え。件数絞りの節を追加

### Added

- **`orientation --knowledge-latest N`** — knowledge 索引を**新しい順 N 件**に絞る（id は日付順に振られるため id の大きい方が新しい＝`--handoff-latest` と同じ読み筋）。見出しは `knowledge (latest N of M records, newest last, ...)` で母数を開示する（category 絞りと同じ規約＝サイレントに減らさない）。**選ぶのは新しい順だが、並びは id 昇順のまま**——索引の読み方は変えず母数だけを減らす。この捻れは見出しの `newest last`（＝末尾が最新）で開示する——**latest 指定時のみ**載り、未指定の見出しは不変。`--knowledge-category` との併用は**絞ってから latest**（M は category 絞り後の件数）。既定は未指定＝全件で後方互換
- **ROUTINE_PROMPT Step 5 への明示投入** — 呼び出し行を `orientation --knowledge-latest 200` へ。**200 は仮置き**（現行全量 192 件を当座のアンカーに置いただけ——`handoff_latest=3` と同じ作法で、次の実測で校正する）
- **knowledge category 統合の移行指示書**（開発記録 `docs/devlog/`、配布対象外） — `knowledge_category_migration_20260809.md`。無検証ゆえ 18 種へ自然増殖し同義の綴り違い（`ops` / `operations`）が混ざった category を寄せるための、統合マップ提案・バックアップ・冪等 update・件数検証・完了報告の 5 点構成。**コード変更は含まない**（移行の実行は秘書の自由時間タスクで自走）。許可集合検証（Identity / Goal と同型の `__post_init__`）は移行完了報告を昇格トリガーとする `cc-defer` として `domain/registry.py` に刻んだ

### Notes

- **既定出力は v1.5.0 / v1.6.0 と byte 同一ではなくなり得る** — 日本語主体のデータでは、字数で数えていた頃より**実効幅が狭まるため出力が縮む**（これが本版の目的そのもの）。ASCII のみの入力では字数＝バイト数ゆえ従来と同値で、幅に収まる入力は単位変更の影響を受けない——v1.5.0 のスナップショットテストはこの**互換契約**として残してある（削除ではなく転用）
- 幅を明示指定している運用があれば、その値は今後バイトとして解釈される（日本語主体なら実効的に狭まる）。必要なら CLI オプションで広げられる
- Step 5 を変更したため、本番へ届けるには **cloud routine の body 再登録が必要**（リポ修正だけでは到達しない）。v1.6.0 の手順 11 / 14 変更分の再登録と同便にできる
...
無防備で、防御の一部が「エージェント運用責務」「設計要件」の記述のまま機械化されていなかった。
本リリースはその 4 点を実装へ落とす。挙動を止める向きの変更は入れていない（検知・redact・
窓による絞りのみ）。

### Added

- **出力漏洩スキャン** — 送信本文に混じった bot token / PAT / 秘匿 env 変数名 / ローカル絶対パスを形状で検出し redact する（`domain/output_scan.py`）。適用点は `SendReply` と `ProactiveSend` の両方（SECURITY.md §4 が両経路を対象と明記していた）。**送信自体は止めない** — 止めると秘書が黙り、事故より障害の方が起きやすい。入力側がフラグ止まりなのに対し出力側で redact するのは、送信が不可逆だから
- **未認可アクセスの記録** — 破棄していた update の `chat_id` と時刻を 1 行残す（本文は載せない）。痕跡が無ければ bot に誰が到達したかを事後に観測できない
- **chat 単位のレート制限** — 認可 chat ごとに直近 60 秒 30 件までを emit する窓（`domain/rate_limit.py`）。allowlist は「誰が」を絞るが「どれだけ」を絞らないため、認可済み端末の暴走送信がエージェント turn を無制限に焚く経路が開いていた。拒否分は履歴に積まない（フラッド中も窓が開く）
...
- **SECURITY.md を実態へ** — 上記で機械化された項目を「運用責務」から実装済みの記述へ改めた
...
- テスト **636 passed**（v1.4.0 の 602 から +34）
