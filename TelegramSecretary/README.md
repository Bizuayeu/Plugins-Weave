...
| `orientation [--notes-tail N] [--topic-width N] [--handoff-latest N] [--handoff-cap N] [--knowledge-category CAT] [--knowledge-latest N]` | 起動時オリエンテーションのダイジェスト（role + 7表の件数/バイト数 + 小表全文 + tasks 一行要約と active の notes 末尾 + knowledge 索引 + handoff 最新ブロック）を emit。幅の単位は UTF-8 バイトで丸めは文字境界、出力はレコード長に依存せず有界（DESIGN §3.12）。`--knowledge-category` で索引を category 完全一致に絞る（見出しの `N of M` に全件数が残る）、`--knowledge-latest` で新しい順 N 件に頭打ち（併用時は絞ってから latest） | 0, 2=設定欠損 |
