...
| **プロファイルあり（PROFILE）** | 執事（嗜好を踏まえた先回り） | 守護霊（人物理解 × 伴走の両輪） |

- **P 軸（パーソナライズ）** — 人物理解の材料は3経路：占術スキル（ABILITIES 登録、例: PrecognitiveViewer）／JSON 出力型占いサイトの紹介（ユーザー自身が取得した JSON を秘書が解釈）／MBTI 等の直接聴取
- **A 軸（伴走）** — 四大相談コース（お金・仕事・人間関係・健康）の目標逆算（GOALS → STEPS）＋ proactive-send ナッジ
- **卒業まで設計済み** — すべての目標が達成（achieved/abandoned）になると A 軸が降り、守護霊は自然に執事へ戻ります。伴走は預かりもの——変容を見届けたら手を離す仕様です（目標を足せば再びコーチ/守護霊へ、可逆）
...
| `lint-numbers <path>` | 納品物・handoff（原稿 md）の裸数値スキャン。数字を含む各行に計器トークン（`実測` / `見込み` / `出典` 等の出所マーカー）が同じ行にあるかを二値判定し JSON 1 行で emit（`path` / `number_lines` / `covered` / `bare` / `bare_lines`）。presence の検査であって正しさの検査ではない。read-only、裸 0 件でも 1 行出す。裸の有無は exit code でなく JSON の値（DESIGN §3.13） | 0=OK（裸の有無に依らず）, 2=不在/不可読 |
...
| `orientation [--notes-tail N] [--topic-width N] [--handoff-latest N] [--handoff-cap N] [--knowledge-category CAT] [--knowledge-latest N] [--knowledge-subject ID] [--profile-cap N] [--individuals-cap N] [--abilities-cap N] [--goals-cap N] [--tasks-latest N] [--steps-latest N] [--artifacts-latest N]` | 起動時オリエンテーションのダイジェスト（role + 8表の件数/バイト数 + outbound の最終送信確定行〔WAL の done 最新 `created_at` と pending 件数、v1.16.0〕 + 小表全文〔goals〕 + individuals / abilities / profile / tasks / subjects / steps の一行索引と active の notes 末尾 + **tasks.notes 直後の `## artifacts`**〔active タスクごとの採択済み成果物のファイル名、`--artifacts-latest N` で群ごとに頭打ち。中身は開かない、v1.17.0〕 + knowledge 索引 + handoff 最新ブロック）を emit。幅の単位は UTF-8 バイトで丸めは文字境界、出力はレコード長に依存せず有界（DESIGN §3.12）。`--knowledge-category` で索引を category 完全一致に絞る（見出しの `N of M` に全件数が残る）、`--knowledge-subject` で subjects の要素一致に絞る、`--knowledge-latest` で新しい順 N 件に頭打ち（併用時は category → subject → latest の順）。`--individuals-cap` / `--profile-cap` / `--abilities-cap` は索引行の自由記述 1 列（`context_notes` / `content` / `guidance`）の頭の幅（individuals は v1.18.0、profile / abilities は v1.19.0、未指定なら 120。件数絞りは無く全件）、`--goals-cap` は goals の notes を丸めるバイト上限（未指定なら全文）、`--tasks-latest` は tasks 一行要約の**終端（done / cancelled 等）だけ**を新しい順 N 件に絞り（active は件数絞りの対象外＝常に全件、v1.15.2。notes も載った集合の active に連動）、`--steps-latest` は steps 索引を新しい順 N 件に絞る（`--steps-latest` は未指定なら全件。索引行の頭の幅 3 つ〔`--individuals-cap` / `--profile-cap` / `--abilities-cap`、未指定なら 120〕を除き、いずれも未指定＝全文・全件。意味論の正典は SKILL.md と DESIGN §3.12）。digest の総バイトは毎回 stderr に `orientation digest: N bytes` として出る（25,600 超は警告付き、stdout・exit code は不変） | 0, 2=設定欠損 |
| `knowledge search --query Q [--query Q2] [--any] [--category C] [--subject S] [--limit N]` | knowledge の read-only 検索（v1.17.0）。id / subjects / topic / content の部分文字列一致（NFKC・大小無視）、複数語は既定 AND・`--any` で OR、絞りは category → subject → query → limit。索引行で返り（本文は `get --key`）、見出しに `N matches of M records` を開示、0 件でも exit 0。総バイトは stderr に申告（25,600 超は警告）。新しい知見を焼く前の既出照合の口（意味論の正典は SKILL.md） | 0, 2=`--query` 無し／knowledge 以外 |
| `artifacts-sync` | 成果物層 `artifacts/`（申し送りの `handoff/` ブロックを含む）を固定ブランチへ commit & push。`registry_sync` 無効・`artifacts/` 未作成は no-op | 0=OK（push 失敗は stdout の `pushed=False`、commit はローカルに残り次回再送）, 1=同期処理の例外 |
| `handoff-archive <name>...` | 消化済みの申し送りブロックを `handoff/archive/` へ移して卒業させる（以後 orientation に載らない）。移動後は `artifacts-sync` 経路で push。不正名・不在・archive 側の同名既存は何も移動せず exit 2 | 0=OK（push 失敗は stdout の `pushed=False`、commit はローカルに残り次回再送）, 1=同期処理の例外, 2=不正/不在 |
