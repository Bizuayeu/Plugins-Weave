...
- **README に動作要件「進捗表ツールの有効化」** — Claude Code v2.1.233 で Todo/task-tracking ツール群（`TodoWrite`、`TaskCreate`/`Get`/`Update`/`List`）が新しい世代のモデル（Opus 4.8 / Sonnet 5 / Fable 5 / Mythos 5 および以降）で既定無効に変更されたため、`/plan-sdd`・`/outsource`・`/dig` と orchestrator が frontmatter で宣言する進捗表ツールが解決しない組み合わせが生じる。有効・無効は CLI とモデルの**両方**で決まるので二軸の表で示し、`CLAUDE_CODE_ENABLE_TODO_TOOLS=1` による有効化手順と、無効のまま運用した場合に失われるもの（進捗表の描画／orchestrator の stall 再開耐性がファイル物証の片肺になること）を明記した。既存の「ネスト生成の許可」と同種の、harness 側のゲートに由来する動作要件である

### Fixed

- **欠落していた `## [1.5.0]` と `## [1.4.0]` の見出しを復元** — 両リリースの `### Added` ブロックが直上の版の見出しに吸収され、`## [1.5.1]` の下に 3 つの `### Added` が並ぶ状態になっていた（1.4.0 の見出しは 1.5.0 リリース時点で既に失われており、欠落は 2 リリース分にわたっていた）。見出し行はプラグインキャッシュに残る各版の CHANGELOG から転記した。本文の追加・改変はなく、ブロックの帰属を元の版へ戻しただけである

## [1.5.1] - 2026-08-08

### Added

- **dev-rules に「先送りは grep 可能に刻む」** — 意図的な仮置き・簡易実装に、天井（何で妥協したか）と昇格トリガー（何が起きたら本実装へ進むか）の二成分を `cc-defer:` コメントとして必須化。散文の「仮置き明示」を機械可読へ格上げし、検収レポートが閉じた後もコード内で負債の意図が生存する（回収は `grep -rn "cc-defer:"` の決定論で足り、収穫コマンドは必要が立ってから）。トリガーの無い先送りは回収の機会を失って腐るため、二成分を規約とする
- **解の探索梯子③にプラットフォーム固有機能を併合** — 「③標準ライブラリ・プラットフォーム固有機能（DB 制約をアプリコードより、CSS を JS より）」。実行環境が既製で持つ力は標準ライブラリと同格の「書かずに済む段」であり、この欠段がフロントエンド・DB 作業での過剰実装の入口だった。③への併合で段数は据え置き、梯子の認知負荷を増やさない

## [1.5.0] - 2026-08-08

### Added

- **dev-rules に Deletion Test（YAGNI の出口側の操作形）** — 成果物の各要素に「これを削っても完了条件の証明は成立するか？」と問い、成立するなら削る検品規則。YAGNI が入口（作る前）の審査であるのに対し、こちらは出口（作った後）の検品で、KISS の「削れる語を削る」をコード成果物へ拡張する。ただし入力検証・エラー処理・セキュリティ対策は削る対象にしない（動くだけの近道は簡潔ではなく未完成）——簡潔化圧が品質の床を抜かないための明示的な下限
- **dev-rules に「根拠なき数値を発明しない」** — タイムアウト・リトライ回数・閾値・上限は出所（依頼元の指定・仕様・実測・プラットフォーム制約）を言える値だけを書く。LLM は尤もらしい数値を無から埋める傾向があり、発明された数値は「なぜこの値か」に誰も答えられない負債として残る。仮置きが避けられない場合は仮置きである旨と選定理由の明示を義務付ける
- **dev-rules の Understand に解の探索梯子** — 書き始める前に ①そもそも書かずに済むか → ②既存コードの再利用 → ③標準ライブラリ → ④導入済みの依存 → ⑤最小の新規実装 の順で解を探す。梯子を一段下りるごとに「その段で済まない理由」を一つ要求し、「まず書く」への既定の傾きを「まず探す」へ反転する
- **worker 報告に完了状態語彙（COMPLETED / PARTIAL / BLOCKED / RETURNED）** — 報告冒頭の一語宣言を規格化し、3-Strike の STOP・中断・検証未了を COMPLETED と書くことを禁じる（止まったことと終わったことは別の状態）。orchestrator 側もブリーフ規格（報告形式の指定に状態宣言を含める）とレビュー規律（状態宣言と物証の整合検分、COMPLETED 以外を完了として集計しない）で同じ語彙を使う——片側だけの語彙は散文報告へ静かに退化するため、両面の対称を構造テストで担保
- **worker に発見の黙過禁止** — スコープ外で気づいた問題は手を出さない代わりに必ず報告へ記載する。発見して黙ることは、検分する側には「問題なし」と映る——手を出さない規律と黙らない規律は別の規律であり、前者だけでは発見が消える

## [1.4.0] - 2026-07-30

### Added

- **検収レポートに二次計画セクション（`{{NEXT_PLAN}}`）** — `/outsource` Phase 5 が、検収で得た所感（差し戻し・保留の上申／クイズが問うたリスク／元計画の Non-goals／物証で観測した技術負債）から次サイクルの意図を plan-sdd の 4 フィールド（Goal / Constraints / Non-goals / Acceptance）で合成し、**コピペ可能な `/ConsiderateCoder:plan-sdd <意図>` ブロック**としてレポート末尾（理解度クイズの後）へ描画する。新規調査は行わず、材料は Phase 4/5 の既存情報に限る。器（テンプレート）は決定論のまま、追加はプレースホルダ 1 個で既存 7 個は非破壊
- **収束宣言と default-to-convergence** — 材料が Acceptance に有意差を生む改善候補を構成しない場合は「**二次計画なし（収束）**」＋理由一行を正規の出力とし、「品質を極限まで高める」のような漸近的ゴールの生成を禁じる。さらに改善提案と収束の判断が拮抗した場合は**収束に倒す**（LLM の加算バイアスに対する傾き。二次計画は毎回の成果物ではなく「出さないことが既定の、例外としての提案」）。性向依存の規則ゆえ、規則文の存在自体を構造テストの近接検査で担保する
- **理解度ゲート付きの裁可接続** — communicator は生成した二次計画で plan-sdd を**自発的に起動しない**。レポート生成の後、二次計画がある場合のみ、二次計画の根拠（どの検収所感が材料か・なぜ収束でないか・Non-goals の境界）を問うテスト設問 2-3 問を `AskUserQuestion` で出し、**全問正解の場合のみ**接続設問（1 問・推奨マークなし）へ進み、承認時だけ `Skill` ツールで `ConsiderateCoder:plan-sdd` を起動する。不正解があれば正答を開示せずレポートの再読を提案して終了、収束時はゲートごと発火しない（「上申事項ゼロなら問わない」と同じ条件付き非対称）。ゲート不合格・保留・却下のいずれでもコピペブロックが残るため手動実行は常に可能——ゲートは主権の制限ではなく、便利な経路にだけ理解の確認を求める摩擦の再配置である。これにより SDD 計画 → 三層委任 → 検収 → 二次計画 → SDD 計画 の**環の全ての縫い目に人間の裁可が常設**される
